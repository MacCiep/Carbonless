...
RSpec.describe Api::HighScoresController do
  describe 'GET #index' do
    it_behaves_like 'protected endpoint', method: :get, url: '/api/high_scores.json'

    context 'when user is logged in' do
      subject { send(:get, api_high_scores_path, headers:, params:) }
      ...
      context 'when city scope is provided' do
        ...
        let(:leader_board) { User.where(city:).order(score: :desc).limit(5) }
      ...
      context 'when country scope is provided' do
        ...
        let(:leader_board) { User.where(country:).sort_by(&:score).last(5).reverse }
